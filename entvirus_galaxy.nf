...
def usage() {
    println("entvirus --in <reads_file> --out <output_dir> --cpus <nb_cpus> --mode <clc,megahit,metacompass,ray,spades> -w <temp_work_dir> --annotated <yes,no> --focus <yes,no>")
}
...
readChannel = Channel.fromPath("${params.in}")
...
process filtering {
    ...
    input:
    set pair_id, forward, reverse from readChannel
