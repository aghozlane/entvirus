...
def get_arguments():
    ...
    parser.add_argument('-p', dest='phylogeny_software', type=str, choices=["FastTree", "IQ-TREE"],
                        default="FastTree", help='Choose which software to perform phylogeny (default FastTree)')
    ...
    parser.add_argument('-o', dest='res_png', type=str, default='result.png', help='Output png file')
    parser.add_argument('-om', dest='output_matrix', type=str, default='matrix.tsv', help='Output matrix file')
    ...
    parser.add_argument('-osm', dest='output_sim_matrix', type=str,
        default='matrix_simuation.tsv', help='Output matrix file')

    return parser.parse_args()


def divide_alignment(aligned_multifasta_file, segment_size, segment_increment_size):
    """
    """
    seq = ""
    #check_size = False
    ...
    return segment, size_alignment, count_seq
...
def extract_segment(aligned_multifasta_file, segment, temp_dir, segment_size, support_threshold, phylogeny_software):
    """
    """
    seg_section = []
    list_run = []
    ...
    try:
        for i in segment:
            ...
            if phylogeny_software == "FastTree":
                list_run.append("FastTree -gtr -gamma -nt {0}.fasta 2> /dev/null |gotree collapse support -s {1} -o {0}.tree".format(name, support_threshold))
            else:
                list_run.append("iqtree -redo -m GTR+I+G4 -s {0}.fasta > /dev/null && gotree collapse support -i {0}.fasta.treefile -s {1} -o {0}.tree".format(name, support_threshold))
...
def plot_heatmap(matrix_dist, output_file, segment_increment_size,
                 size_alignment, count_seq, max_val):
    """
    """
    fig, ax = plt.subplots()
    ...
    sns_plot = sns.heatmap(matrix_dist, xticklabels=10, yticklabels=10,
                           square=True, mask=mask, vmin=0, vmax=1.0)
    sns_plot.invert_yaxis()
    sns_plot.set_xticklabels(np.arange(0, size_alignment + segment_increment_size, 1000))
    sns_plot.set_yticklabels(np.arange(0, size_alignment + segment_increment_size, 1000))
    #sns.axes_style("white")
    #sns_plot.set_yticks(all_posit)
    sns_plot.set_title('Phylogenetic compatibility matrix\n(n={}, max value={:.2g})'.format(count_seq, max_val))
...
def run_computation(aligned_multifasta_file, segment_size, segment_increment_size,
    temp_dir, phylogeny_software, support_threshold, num_cpu, output_file, matrix_file):
    # Extract alignment
    segment, size_alignment, count_seq = divide_alignment(
        aligned_multifasta_file, segment_size, segment_increment_size)
    list_run, list_tree = extract_segment(aligned_multifasta_file, segment,
        temp_dir, segment_size, support_threshold, phylogeny_software)
...
def main():
    ...
    if not os.path.isdir(temp_dir):
        os.mkdir(temp_dir)
    size_alignment = run_computation(args.aligned_multifasta_file, args.segment_size,
        args.segment_increment_size, temp_dir, args.phylogeny_software,
        args.support_threshold, args.num_cpu, args.res_png, args.output_matrix)
    # Run simulation
    print("Let's go for simulation")
    output_name = args.temp_dir + os.path.splitext(os.path.basename(args.aligned_multifasta_file))[0]
    if args.tree_file:
        run_command("{4}/seq-gen -l {2} -of -m GTR {0} | mafft  --thread {3} --maxiterate 1000 --localpair - > {1}.ali".format(args.tree_file, output_name, size_alignment, args.num_cpu, os.path.dirname(os.path.abspath(__file__))))
    else:
        run_command("iqtree -nt AUTO -m GTR+I+G4 -s {0} -redo > /dev/null && {4}/seq-gen -l {2} -of -m GTR {0}.treefile | mafft  --thread {3} --maxiterate 1000 --localpair - > {1}.ali".format(args.aligned_multifasta_file, output_name, size_alignment, args.num_cpu, os.path.dirname(os.path.abspath(__file__))))
    # run computation for simulation data
    temp_dir = args.temp_dir + os.sep + "simulation/"
    if not os.path.isdir(temp_dir):
        os.mkdir(temp_dir)
    run_computation(output_name + ".ali", args.segment_size,
        args.segment_increment_size, temp_dir, args.phylogeny_software,
        args.support_threshold, args.num_cpu, args.sim_png, args.output_sim_matrix)
